...
import { getDir, error } from './utils.ts';
import { process } from './build.ts';
...
await new Command()
  ...
  .action(async ({ output }, input) => {
    const processResult = await process(getDir(input), getDir(output));
    if (processResult.isSome) return error(processResult.value);
    console.log('Success');
  })
  .parse(Deno.args);
